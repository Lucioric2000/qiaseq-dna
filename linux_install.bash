...
echo -e "#Shell environment for qiagen\nexport PYTHONPATH=$PYTHONPATH:/srv/qgen/code/qiaseq-dna/\nexport LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/srv/qgen/bin/ssw/src/">/etc/profile.d/qiagen.sh
#Calls thiss script
bash -c /etc/profile.d/qiagen.sh
#Declare the location of the conda installaction
conda_home=/home/bitnami/miniconda2
#Make the directories if don't exist
sudo mkdir -p /srv/qgen/code && \
    sudo mkdir -p /srv/qgen/bin/downloads && \
    sudo mkdir -p /srv/qgen/data/genome && \
    sudo mkdir -p /srv/qgen/data/annotation && \
    sudo mkdir -p /srv/qgen/example/
...
conda install bedtools=2.25.0 htslib=1.3.1 cutadapt=1.10 snpeff=4.2 bwa=0.7.15
#Picard 1.97 wa not found in the default conda ditribution
#wget https://storage.googleapis.com/qiaseq-dna/lib/ssw.tar.gz https://storage.googleapis.com/qiaseq-dna/lib/fgbio-0.1.4-SNAPSHOT.jar -P /srv/qgen/bin/
...
#wget https://storage.googleapis.com/qiaseq-dna/lib/py-editdist-0.3.tar.gz https://storage.googleapis.com/qiaseq-dna/lib/sendgrid-v2.2.1.tar.gz -P /srv/qgen/bin/downloads/
...
#wget https://github.com/samtools/samtools/releases/download/1.5/samtools-1.5.tar.bz2 -O /srv/qgen/bin/downloads/samtools-1.5.tar.bz2 && \
    #cd /srv/qgen/bin/downloads/ tar -xvf samtools-1.5.tar.bz2 && \
    #cd samtools-1.5  && mkdir -p /srv/qgen/bin/samtools-1.5 && ./configure --prefix /srv/qgen/bin/samtools-1.5 && make && make install
...
################ TVC binaries ################
sudo mkdir -p /srv/qgen/bin/TorrentSuite/
...
sudo chmod 775 /srv/qgen/bin/TorrentSuite/tmap /srv/qgen/bin/TorrentSuite/tvc
...
sudo mkdir -p /srv/qgen/test_smcounter-v2/
